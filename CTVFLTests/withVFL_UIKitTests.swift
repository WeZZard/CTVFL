...
class withVFL_UIKitTests: XCTestCase {
    ...
    func testCanCompile_withLayoutGuideAndViewWithSystemSpacing() {
        if #available(iOS 11.0, tvOS 11.0, *) {
            ...
            XCTAssertEqual(h[0].firstAnchor, view1.leadingAnchor)
            XCTAssertEqual(h[0].secondAnchor, rootView.safeAreaLayoutGuide.leadingAnchor)
            ...
            XCTAssertEqual(v[0].firstAnchor, view1.topAnchor)
            XCTAssertEqual(v[0].secondAnchor, rootView.safeAreaLayoutGuide.topAnchor)
    ...
    func testCanCompile_withViewAndLayoutGuideWithSystemSpacing() {
        if #available(iOS 11.0, tvOS 11.0, *) {
            ...
            XCTAssertEqual(h.count, 1)
            XCTAssertEqual(h[0].firstAnchor, rootView.safeAreaLayoutGuide.trailingAnchor)
            XCTAssertEqual(h[0].secondAnchor, view1.trailingAnchor)
            XCTAssertEqual(h[0].relation, .equal)
            XCTAssertEqual(h[0].constant, 8)
            XCTAssertEqual(h[0].multiplier, 1)
            XCTAssertEqual(h[0].priority, .required)
            ...
            XCTAssertEqual(v.count, 1)
            XCTAssertEqual(v[0].firstAnchor, rootView.safeAreaLayoutGuide.bottomAnchor)
            XCTAssertEqual(v[0].secondAnchor, view1.bottomAnchor)
            XCTAssertEqual(v[0].relation, .equal)
            XCTAssertEqual(v[0].constant, 8)
            XCTAssertEqual(v[0].multiplier, 1)
            XCTAssertEqual(v[0].priority, .required)
